...
Meter renders in one of two skins, declared per page on the root element:
...
`Meter` set in the serif, followed by `˘ ˉ ˘ ˉ` in mono at 0.75rem: scansion
marks for unstressed, stressed, unstressed, stressed. It is the single place the
data metaphor is allowed to be literal, and it does not appear anywhere else —
no charts of stress patterns, no syllable counters. The metaphor sets the tone
...
2. Copy the header and colophon from the nearest existing page of that skin. The
   ...
   a poem.
3. Relative links only. The site is mounted at `datarail.org/meter` today and
